...
class Approacher():
	...
	def approach(self, locx,locy,tip):
		...
		if(tip == 'obraz'):
			modifier=0
			distaway=0.50 #nism cist sure kk dalec stran od obraza mora bit da lepo zazna can be changed if needed
		...
		for i in range(8):
			...
			if(check!=[]):
				...
				if(tip=='cylinder'):
					...
					print("roka")
				#self.moveBack(0.1*modifier) #move back by calling doMoveBack instead
				break #uncomment for all approaches
			else:
...
if __name__ == "__main__":
	rospy.init_node("navnode1")
	a = Approacher()
	a.approach(2.9,0.46,'cylinder') #cylinder
	#a.approach(-0.5,0.0,cylinder) #neutral
	#a.approach(-1.83,-0.1,'cylinder') #cylinder
	#a.approach(-1.33,0.25,'ring') #ring
	#a.approach(1.5,2.8,'obraz')
