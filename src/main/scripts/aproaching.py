...
def approach(locx,locy,tip):
	...
	for i in range(8):
	    check=checkGoal(0.0,0.0,locx+ring1x[i],locy+ring1y[i])
	    if(check!=[]):
	        print("goal",locx+ring1x[i]," ",locy+ring1y[i],"reachable")
	        moveTo(locx+ring1x[i],locy+ring1y[i],rotations[i])
	        if(i%2==1):
	            modifier=modifier*1.4	
	        moveForward(0.1*modifier)
	        rospy.sleep(2)
	        if(tip=='cylinder'):
